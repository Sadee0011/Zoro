...
//=======================================
    ...
    try {
        const text = (msg.message.conversation || msg.message.extendedTextMessage.text || '').trim();
        const parts = text.split(" ").slice(1).trim().split(/\s+/);
        
        if (parts.length < 2) {
            await socket.sendMessage(sender, { 
                text: '*🚫 Usage: .csong <channel_id> <song_name>\n📌 Example: .csong 120363420985544024@newsletter sadu kare*'
            });
            return;
        }
        ...
        if (!channelId.endsWith('@newsletter') && !channelId.endsWith('@s.whatsapp.net')) {
            await socket.sendMessage(sender, { 
                text: '*❌ Invalid channel ID format! Use: number@newsletter or number@s.whatsapp.net*'
            });
            return;
        }
        ...
        await socket.sendMessage(sender, { 
            text: '*🔍 Searching for song and sending to channel...*'
        });
        ...
        await socket.sendMessage(channelId, {
            image: { url: thumbnail },
